...
def live_publisher_task():
    ...
    while True:
        try:
            ...
            print(f"[DUS1] Live publish error: {e}")
...
def door_ultrasonic_sensor_one_callback(distance, settings):
    ...
    live_queue.put(('Door Distance Sensor 1/live', json.dumps(payload)))

    # 2. Dodaj u batch (za InfluxDB) - šalje daemon nit
    with counter_lock:
        ultrasonic_batch.append(('Door Distance Sensor 1/batch', json.dumps(payload), 0, True))
...
def run_door_ultrasonic_sensor_one(settings, threads, stop_event):
    if settings['simulated']:
        ...
        door_ultrasonic_sensor_one_thread = threading.Thread(target = run_door_ultrasonic_sensor_one_simulator, args=(4, door_ultrasonic_sensor_one_callback, stop_event, settings))
