...
def door_ultrasonic_sensor_two_callback(distance, settings):
    ...
    with counter_lock:
        ultrasonic_batch.append(('Door Distance Sensor 2', json.dumps(ultrasonic_payload), 0, True))
...
def run_door_ultrasonic_sensor_two(settings, threads, stop_event):
    if settings['simulated']:
        ...
        door_ultrasonic_sensor_two_thread = threading.Thread(target = run_door_ultrasonic_sensor_two_simulator, args=(5, door_ultrasonic_sensor_two_callback, stop_event, settings))
